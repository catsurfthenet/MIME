...
if(config.test):
    ...
    #model = Train_MIME(vocab, decoder_number=program_number, model_file_path=config.save_path, is_eval=True)
    if (config.USE_CUDA):
        model.cuda()
    model = model.eval()
...
try:
    ...
    for n_iter in tqdm(range(1000000)): #1000000
        temp = next(data_iter)
        ...
        if ((n_iter + 1) % check_iter == 0):
            ...
            model = model.train()
            if (config.model == "mimic" and n_iter < 10000): #10000
                continue
            if (ppl_val <= best_ppl):
                best_ppl = ppl_val
                patient = 0
                model.save_model(best_ppl, n_iter, 0, 0, bleu_score_g, bleu_score_b, bleu_score_t)
                print("Model saved at iter: " + str(n_iter + 1))
                weights_best = deepcopy(model.state_dict())
            else:
                patient += 1
            if (patient > 2): break
